                    ...
                    <div class="product-card" 
                        ...
                        <div class="product-info">
                            ...
                            <div class="product-actions">
                                ...
                                <button class="btn-cart" onclick="agregarAlCarrito(<?php echo $repuesto['id']; ?>, event)">
  ...
  <script>
    ...
    function actualizarProductosFiltrados() {
        ...
        todosLosProductos.forEach(producto => {
            ...
            if (modelo && productoModelo !== modelo) mostrar = false;
            if (anio && productoAnio !== anio) mostrar = false;
        ...
        if (contador) {
            if (categoria || marca || modelo || anio) {
                contador.innerHTML = `Mostrando <strong>${totalEncontrados} de ${totalProductosOriginal} repuestos</strong> disponibles (filtrados)`;
            } else {
                contador.innerHTML = `Mostrando <strong>${totalEncontrados} repuestos</strong> disponibles`;
            }
        }
    ...
    function filtrarProductos() {
        ...
        if (productosFiltrados.length === 0) {
            if (window.Toast && typeof Toast.warning === 'function') {
                Toast.warning('No se encontraron repuestos con los filtros seleccionados', 'Sin resultados');
            }
        } else if (productosFiltrados.length < totalProductosOriginal) {
            if (window.Toast && typeof Toast.info === 'function') {
                Toast.info(`Se encontraron ${productosFiltrados.length} repuestos`, 'Filtros aplicados');
            }
        }
    }
    ...
    function limpiarFiltros() {
        ...
        const contador = document.querySelector('.products-count');
        if (contador) {
            contador.innerHTML = `Mostrando <strong>${totalProductosOriginal} repuestos</strong> disponibles`;
        }
    ...
    // Búsqueda por referencia (si tienes este campo en el HTML)
    function buscarPorReferencia() {
        ...
        if (!busqueda) {
            if (window.Toast && typeof Toast.warning === 'function') {
                Toast.warning('Por favor, ingresa una referencia o nombre para buscar', 'Búsqueda requerida');
            }
            return;
        }
        ...
        if (productosFiltrados.length === 0) {
            if (window.Toast && typeof Toast.warning === 'function') {
                Toast.warning('No se encontraron repuestos con la búsqueda: ' + busqueda, 'Sin resultados');
            }
        } else {
            if (window.Toast && typeof Toast.success === 'function') {
                Toast.success(`Se encontraron ${productosFiltrados.length} repuestos`, 'Búsqueda completada');
            }
        }
    }
