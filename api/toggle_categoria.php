<?php
// /inversiones-rojas/api/toggle_categoria.php
...
try {
    ...
    $stmt = $conn->prepare("UPDATE categorias SET estado = ?, updated_at = NOW() WHERE id = ?");
    $stmt->execute([$estado, $id]);

    if ($stmt->rowCount() === 0) { echo json_encode(['ok'=>false,'error'=>'Registro no encontrado']); exit; }

    echo json_encode(['ok'=>true,'message'=>'Estado de categoría actualizado correctamente']);
} catch (Exception $e) {
    error_log('toggle_categoria: ' . $e->getMessage());
