    ...
    if (!$sock) {
        return [
            ...
            'message' => "No se pudo conectar a {$host}:{$port} — {$errstr} ({$errno}). "
                       . "Verifica que XAMPP tiene openssl habilitado en php.ini "
                       . "(extension=openssl debe estar sin punto y coma)"
        ];
    }
        ...
        if ($port === 587 || $secure === 'tls') {
            ...
            if (!stream_socket_enable_crypto($sock, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
                throw new RuntimeException(
                    "No se pudo activar TLS. Asegura que en php.ini de XAMPP "
                    . "extension=openssl esta habilitado (sin punto y coma al inicio)"
                );
            }
